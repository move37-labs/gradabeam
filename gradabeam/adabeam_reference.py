...
Paper-result AdaBeam designer, ported from NucleoBench.

Source: nucleobench/optimizations/ada/adabeam/adabeam.py
blob 767402c810da5b8df45f81b3238897d7bb194af0
"""
...
class AdaBeamReference:
    """NucleoBench / paper-result AdaBeam designer.

    This class preserves the original optimization trajectory, including
    initialization, set-based candidate collection, and stable fitness
    sorting without a pre-shuffle. Prefer :class:`~gradabeam.AdaBeam` for
    ...
    def __init__(
        ...
        # Paper AdaBeam seeds the first mutate from an unevaluated root.
        seed_node = RolloutNode(seq=start_sequence, fitness=None)  # type: ignore[arg-type]
    ...
    def run(self, n_steps: int):
        ...
        if self.debug and n_steps > 0:
            print(
                f"Step {_step} current scores: {sorted([x.fitness for x in self.current_nodes], reverse=True)}"
            )
    ...
    def propose_sequences(self, root_nodes: list[RolloutNode]) -> list[RolloutNode]:
        ...
        ranked = sorted(sequences, key=lambda x: x.fitness, reverse=True)
        top_nodes = ranked[: self.beam_size]

        if self.debug:
            print(f"Rollout lengths: {rollout_lengths}")
            print(f"Number of candidate sequences: {len(ranked)}")
