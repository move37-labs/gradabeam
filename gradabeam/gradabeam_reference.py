...
Paper-result GradaBeam designer, ported from NucleoBench.

Source: nucleobench/optimizations/ada/gradabeam/gradabeam.py
blob 569ea54af6331d07edcd4bb294cc09186b451824
"""
...
class GradaBeamReference:
    """NucleoBench / paper-result GradaBeam designer with PBT.

    This class preserves the original optimization trajectory, including
    TISM initialization, sampler caching, PBT updates, fitness dtypes, and
    stable fitness sorting without a pre-shuffle. Prefer
    :class:`~gradabeam.GradaBeam` for the current implementation.
    """
    ...
    def propose_sequences(self, root_nodes: list[RolloutNode]) -> list[RolloutNode]:
        ...
        ranked = sorted(nodes_visited, key=lambda x: x.fitness, reverse=True)
    ...
    def mutate_nodes_gradabeam(
        ...
        return [
            RolloutNode(
                seq=seq,
                fitness=float(f),  # type: ignore[arg-type]
                probs=probs,
