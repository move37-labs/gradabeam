...
def step1_real_bpnet():
    ...
    print(
        f"  {N_WARMUP} warm-up steps, then {N_BLOCKS}x{N_BLOCK}-step timed blocks",
        flush=True,
    )
    print("=" * 70, flush=True)
    try:
    ...
    for cls, kw_tmpl, label in [
        ...
        print(
            f"{label}: warm-up done.  Running {N_BLOCKS} blocks of {N_BLOCK} steps ...",
            flush=True,
        )
        ...
        for b in range(N_BLOCKS):
            ...
            print(
                f"  block {b + 1}: {mean_s:.4f} s/step  ({elapsed:.2f} s total)",
                flush=True,
            )
        ...
        print(
            f"  --> grand mean: {grand:.4f} s/step  block-spread: {spread:.4f}  stdev: {stdev:.4f}",
            flush=True,
        )
        results[label.strip()] = {
            "mean": grand,
    ...
    print(
        f"  {'Algorithm':<12} {'mean s/step':>12} {'stdev':>8} {'block-spread':>13}",
        flush=True,
    )
    print("  " + "-" * 48, flush=True)
    for alg, r in results.items():
        print(
            f"  {alg:<12} {r['mean']:>12.4f} {r['stdev']:>8.4f} {r['spread']:>13.4f}",
            flush=True,
        )
    return results
...
def _run_dummy(cls, kw_tmpl, model, label):
    ...
    for b in range(N_BLOCKS):
        ...
        block_means.append(mean_s)
        print(
            f"  block {b + 1}: {mean_s * 1000:.4f} ms/step  ({elapsed:.3f} s total)",
            flush=True,
        )
    ...
    print(
        f"  --> grand mean: {grand * 1000:.4f} ms/step  spread: {spread * 1000:.4f} ms",
        flush=True,
    )
    ...
    if n_fwd_after is not None:
        ...
        print(
            f"  n_forward/step : {fwd_delta / total_steps:.3f}  (total {fwd_delta})",
            flush=True,
        )
        print(
            f"  n_backward/step: {bwd_delta / total_steps:.3f}  (total {bwd_delta})",
            flush=True,
        )
    else:
        print("  (n_forward not available on this branch)", flush=True)
